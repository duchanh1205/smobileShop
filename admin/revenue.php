...
foreach (selectAll("SELECT * FROM donhang WHERE status=3 ORDER BY tongtien DESC LIMIT $item_per_page OFFSET $offset ") as $row) {
    ...
    foreach (selectAll("SELECT * FROM taikhoan WHERE id = '$idtaikhoan1'") as $rows) {
        ...
        if (in_array($idtaikhoan1, $appearedTaikhoanIds)) {
            continue;
        }

        $appearedTaikhoanIds[] = $idtaikhoan1; 
?>
    <tr class="addfont">
        <td><?= $stt++ ?></td>
        <td>
        ...
        <td><?= $rows['taikhoan'] ?></td>
        <td><?= $rows['phanquyen'] == 1 ? 'Admin' : 'Khách hàng'?></td>
        <td><?= rowCount("SELECT * FROM donhang WHERE status=3 && id_taikhoan = $idtaikhoan1") ?></td>
        <td>
            <?php 
            $tongTienCuaTaiKhoan = 0;
            foreach (selectAll("SELECT * FROM donhang WHERE status = 3 && id_taikhoan = $idtaikhoan1") as $item) {
                $tongTienCuaTaiKhoan += $item['tongtien'];
            }
            ?>
            <?= number_format($tongTienCuaTaiKhoan) ?>đ
        </td>
    </tr>
<?php
    }
