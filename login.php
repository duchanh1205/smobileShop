                        ...
                        <div class="login_part_form_iner">
                            ...
                            <?php
                                if (isset($_POST["dangnhap"])) {
                                    ...
                                    //  } 
                                    //  elseif (rowCount("SELECT * FROM taikhoan WHERE taikhoan !='$email' || matkhau !='$matkhau'") > 0){
                                    //      $error = 'Tài khoản hoặc mật khẩu không chính xác. Vui lòng kiểm tra lại';
                                    //  }
                                    //  elseif (rowCount("SELECT * FROM taikhoan WHERE status =1") == 1){
                                    //     $error = 'Tài khoản của bạn đã bị khóa. Vui lòng liên hệ admin để mở khóa tài khoản';
                                    // }
                                    
                                    // Kiểm tra tài khoản bị khóa
    if (rowCount("SELECT * FROM taikhoan WHERE taikhoan = '$email' AND status = 1") == 1) {
        $error = 'Tài khoản của bạn đã bị khóa. Vui lòng liên hệ admin để mở khóa tài khoản';
    } 
    // Kiểm tra tài khoản và mật khẩu chính xác
    elseif (rowCount("SELECT * FROM taikhoan WHERE taikhoan = '$email' AND matkhau = '$matkhau' AND status = 0") == 1) {
        setcookie('user', $email, time() + (86400 * 30), "/");
        header('location:index.php');
    } 
    // Kiểm tra tài khoản hoặc mật khẩu không chính xác
    else {
